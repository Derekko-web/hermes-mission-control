...
export interface CharacterPlacementSnapshot {
  id: number;
...
export class OfficeState {
  ...
  snapshotCharacterPlacements(): CharacterPlacementSnapshot[] {
    return [...this.characters.values()]
      .filter((ch) => !ch.isSubagent && ch.matrixEffect !== 'despawn')
      .map((ch) => ({
        id: ch.id,
        state: ch.state,
        dir: ch.dir,
        ...
        tileCol: ch.tileCol,
        tileRow: ch.tileRow,
        path: ch.path.map((p) => ({ col: p.col, row: p.row })),
        moveProgress: ch.moveProgress,
        currentTool: ch.currentTool,
        palette: ch.palette,
        hueShift: ch.hueShift,
        frame: ch.frame,
        frameTimer: ch.frameTimer,
        wanderTimer: ch.wanderTimer,
        wanderCount: ch.wanderCount,
        wanderLimit: ch.wanderLimit,
        isActive: ch.isActive,
        seatId: ch.seatId,
        bubbleType: ch.bubbleType,
        bubbleTimer: ch.bubbleTimer,
        seatTimer: ch.seatTimer,
  ...
  restoreCharacterPlacements(snapshots: CharacterPlacementSnapshot[]): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      ch.state = snapshot.state;
      ch.dir = snapshot.dir;
      ch.x = snapshot.x;
      ch.y = snapshot.y;
      ch.tileCol = snapshot.tileCol;
      ch.tileRow = snapshot.tileRow;
      ch.path = snapshot.path.map((p) => ({ col: p.col, row: p.row }));
      ch.moveProgress = snapshot.moveProgress;
      ch.currentTool = snapshot.currentTool;
      ch.palette = snapshot.palette;
      ch.hueShift = snapshot.hueShift;
      ch.frame = snapshot.frame;
      ch.frameTimer = snapshot.frameTimer;
      ch.wanderTimer = snapshot.wanderTimer;
      ch.wanderCount = snapshot.wanderCount;
      ch.wanderLimit = snapshot.wanderLimit;
      ch.isActive = snapshot.isActive;
      ch.bubbleType = snapshot.bubbleType;
      ch.bubbleTimer = snapshot.bubbleTimer;
      ch.seatTimer = snapshot.seatTimer;
      ch.folderName = snapshot.folderName;
      ch.inputTokens = snapshot.inputTokens;
      ch.outputTokens = snapshot.outputTokens;
      ch.matrixEffect = null;
      ch.matrixEffectTimer = 0;
      ch.matrixEffectSeeds = [];
      ch.seatId = snapshot.seatId && this.seats.has(snapshot.seatId) ? snapshot.seatId : null;
      if (ch.seatId) {
        const seat = this.seats.get(ch.seatId)!;
        if (seat.assigned) {
          ch.seatId = null;

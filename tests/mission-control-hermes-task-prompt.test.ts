...
test("builds Hermes task run prompts that do work without updating card fields", () => {
  ...
  assert.match(prompt, /Do not change the card status, move the card to Done, update the Kanban column, or change the Notion status/);
  assert.match(prompt, /user will review your work and manually drag the card to Done/);
